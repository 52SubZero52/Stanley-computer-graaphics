...
namespace Game
{
	...
	double pad_left_velocity;
	double pad_right_velocity;
	...
	void init()
	{
		...
		for (int i = 0; i < n_bricks; i++)
		{
			...
			brick_pos[i].y = rand() % screen_height/2;
			brick_state[i] = 1;
	...
	void update()
	{
		...
		pad_pos.x -= pad_left_velocity;
		pad_pos.x += pad_right_velocity;
		...
		for (int i = 0; i < pad; i++)
		{
			...
			if (pad_pos.x < 0 || pad_pos.x + pad_size.w > screen_width)
			{
				pad_speed.x *= -1;
			}
		...
		for (int i = 0; i < pad; i++)
		{
			...
			int result = Collision::minkowski(
				ball_pos.x, ball_pos.y, ball_size.w, ball_size.h,
				pad_pos.x, pad_pos.y, pad_size.w, pad_size.h);
	...
	void movePaddleLeft()
	{
		pad_left_velocity = 0.32;
		pad_right_velocity = 0;
	}

	void movePaddleRight()
	{
		pad_right_velocity = 0.32;
		pad_left_velocity = 0;
	}
	...
	void draw_Pad()
	{
		SDL_SetRenderDrawColor(renderer, pad_color.r, pad_color.g, pad_color.b, 255);

		for (int i = 0; i < pad; i++)
		{

			SDL_Rect rect = { pad_pos.x,pad_pos.y,pad_size.w,pad_size.h };
			SDL_RenderFillRect(renderer, &rect);
		}
